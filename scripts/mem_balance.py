...
"""
    ...
    def _run_experiment(self, workload: str, pinning: str, scheduler: str = "default") -> Dict:
    ...
    def _parse_stress_yaml(self, yaml_file: str) -> Optional[Dict]:
    ...
    def _parse_perf_json(self, json_file: Path) -> Dict:
    ...
    def _create_plots(self, df: pd.DataFrame, max_cpu_persec: float, max_mem_persec: float):
        ...
        for i, pinning in enumerate(pinning_strategies):
            ...
            bars1 = ax1.bar(x_offset, cpu_values, width, label=f'CPU ({pinning})', 
                          alpha=0.8, color=f'C{i}')
            bars2 = ax1.bar(x_offset, mem_values, width, bottom=cpu_values, 
